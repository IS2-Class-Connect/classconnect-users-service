...
export class UserController implements IController<User> {
  ...
  async increaseFailedAttempts(@Param('uuid') userId: string): Promise<User> {
    return await this.userService.increaseFailedAttempts(userId);
  }
  ...
  async checkLockStatus(
    ...
    try {
      const isLocked = await this.userService.isAccountLocked(userId);
